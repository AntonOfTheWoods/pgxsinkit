...
- **Regenerating the demo/integration migrations is entirely the agent's job — there is no maintainer step, ever.** `infra/drizzle/` and `infra/board-drizzle/` only ever target **ephemeral** databases (the demo stacks + the integration/perf harness, recreated fresh on each run); pgxsinkit has **no long-living dev/smoke database**. After a schema/registry change, regenerate the affected migrations yourself — `db:generate` / `db:generate:governance` / `sync:function:generate` / `db:board:generate` / `db:board:sync-fn` (all filesystem-only) — then `format:write`, `validate`, `sync:function:check`, and commit. **Never tell the maintainer to apply, migrate, or reset a database** — the new history applies automatically on the next `infra:up` / `infra:harness:up` / integration run. The only optional follow-up is the agent's own: if a stack is **already running** and must reflect the change now, cycle it (`infra:down && infra:up`, or the `infra:harness:*` pair); **if nothing is running, there is nothing to do** — do not mention applying at all. Full procedure: [docs/runbooks/regenerate-migrations.md](docs/runbooks/regenerate-migrations.md).
