...
// @ts-nocheck -- vendored upstream e2e test; run as a behavioural oracle against real
// Electric+Postgres, deliberately NOT type-audited under this repo's strict tsgo config.
/**
 * Conformance baseline (e2e) — VENDORED from electric-sql/pglite,
 * packages/pglite-sync/test-e2e/sync-e2e.test.ts
 ...
 * `BEGIN`/`COMMIT` statements run on one connection instead of tripping Bun.SQL's pooled-transaction
 * guard (ERR_POSTGRES_UNSAFE_TRANSACTION). */
function makePgClient(url: string) {
  const sql = new SQL({ url, max: 1 });
  // node-`pg` binds a JS array straight into an array column; Bun.SQL does not, so render arrays
  // as a Postgres array literal (`{1,2,3}`) and let the column type coerce it. (JSON objects bind
  // fine, so they pass through untouched.)
  const toPgParam = (value: unknown): unknown => {
    if (!Array.isArray(value)) {
      return value;
    }
    const element = (v: unknown): string => {
  ...
  return {
    query: (text: string, params?: unknown[]) =>
      params && params.length > 0 ? sql.unsafe(text, params.map(toPgParam)) : sql.unsafe(text),
    connect: async () => {},
    end: async () => {
      await sql.end();
    },
...
const fetchClient: typeof fetch = async (url: string | Request | URL, options: RequestInit = {}) => {
...
describe("sync-e2e", () => {
  let pgClient: ReturnType<typeof makePgClient>;
  let pg: PGlite & PGliteInterfaceExtensions<{ electric: ReturnType<typeof electricSync> }>;

  // Setup PostgreSQL client and tables
  beforeAll(async () => {
    // Connect to PostgreSQL (Bun.SQL behind a node-`pg`-shaped shim)
    pgClient = makePgClient(DATABASE_URL);
    ...
    await pgClient.query(`DROP TRIGGER IF EXISTS test_syncing_trigger ON test_syncing;`);
    await pgClient.query(`
  ...
  it("performs operations within a transaction", async () => {
    ...
    const lastItem = await pg.sql`SELECT * FROM todo WHERE id = ${numInserts - 1};`;
  ...
  it("uses COPY FROM for initial batch of inserts", async () => {
    ...
    const values = Array.from({ length: numInserts }, (_, idx) => `(${idx}, 'task${idx}', ${idx % 2 === 0})`).join(
      ", ",
    );
    await pgClient.query(`
      INSERT INTO todo (id, task, done)
...
newline', false);
  ...
  it("uses the specified metadata schema for subscription metadata", async () => {
    ...
    const schemaResult = await pg.query(`SELECT schema_name FROM information_schema.schemata WHERE schema_name = $1`, [
      metadataSchema,
    ]);
    ...
    const subscriptionResult = await pg.query(`SELECT * FROM ${metadataSchema}.subscriptions_metadata WHERE key = $1`, [
      "custom_schema_test",
    ]);
  ...
  it("handles transactions across multiple tables with syncShapesToTables", async () => {
    ...
    const updatedProjectResult = await pg.sql`SELECT * FROM project WHERE id = 1;`;
  ...
  const many_syncer = async (
    ...
    const middleRow = await pg.sql`SELECT * FROM todo WHERE id = ${middleRowId};`;
