...
`authUid` is removed (pre-launch, no legacy). An `authClaim` field declares:
...
- **No behaviour change for the `sub` case.** `authClaim ["sub"]` cast to `uuid` is byte-equivalent in
  effect to the old `auth.uid()` stamp (the function sets `request.jwt.claim.sub` from the same claims), so
  every existing owner/author column stamps the same value as before.
